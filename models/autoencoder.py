...
class Autoencoder(nn.Module):
    ...
    def test(self):
        ...
        tp = joined_df.filter((col("intrusion") == 1) & (col("predicted_intrusion") == 1)).count()
        tn = joined_df.filter((col("intrusion") == 0) & (col("predicted_intrusion") == 0)).count()
        fp = joined_df.filter((col("intrusion") == 0) & (col("predicted_intrusion") == 1)).count()
        fn = joined_df.filter((col("intrusion") == 1) & (col("predicted_intrusion") == 0)).count()
        ...
        result_df_model = result_df_model.drop("row_idx")

        # NOTE: uncomment these two lines if you want to generate a CSV file with the dataframe with the
        # label generated by AE
        csv_filename = f'./generatedDatasets/labeled_autoencoder_output_device_{self.__dev_addr}_{self.__dataset_type.value["name"]}.csv' \
            if self.__dataset_type is not None else f'./generatedDatasets/labeled_autoencoder_output_device_{self.__dev_addr}.csv'
        
        result_df_model.drop("features").toPandas().to_csv(csv_filename, index=False)

        # Step 9: Compute accuracy from joined_df (before modifying the 'intrusion' column)
        correct_preds = joined_df.withColumn(
            "correct",
            when(col("intrusion") == col("predicted_intrusion"), 1).otherwise(0)
        ).agg({"correct": "sum"}).collect()[0][0]

        total = joined_df.count()
        accuracy = correct_preds / total if total > 0 else 0.0

        return result_df_model, accuracy, confusion_matrix